...
class SO2EquivariantGraphAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.use_atom_edge_embedding:
            self.source_embedding = nn.Linear(self.edge_feature_in, self.edge_channels_list[-1])
            self.target_embedding = nn.Linear(self.edge_feature_in, self.edge_channels_list[-1])
...
class TransBlockV2(torch.nn.Module):
    ...
    def __init__(
        ...
        self.ga = SO2EquivariantGraphAttention(
            ...
            SO3_grid=SO3_grid, 
            edge_feature_in=edge_feature_in,
